...
// We don't use pid values at this time
     ...
     * per minute The angular range is about 270 degrees as servo inpute goes
     * from 0 to 1 which translates into about 0.0037 servo units per period.
     * This is ath maximum velocity of 50 degrees per second, way too fast.
     * Propose slowing it down by a factor of 4, so the incremental adjustment
     * is about .0009. A reason able error value is 1 degree say about 0.005 PID
     ...
     *
     * @param target
     * @return has servo hit target within error of 10% above servoError .005
     * about 1.5 degrees
     */
    ...
    /**
     * This method deals with two problems in setting the servos that control
     * the rod angle. 1.Fast jerky servo movements- it slows the servo down 2.
     * The servo goes to its target position immediately and would therefore
     * pass any test I can think of it being finished. In a parallel that
     ...
     * since the tapeangle command will be finished at a tapelength that is not
     * the final target. Meanwhile, the pulley would continue for several more
     * seconds command to its final tape length, This approach is to increment
     * the servo value by a small amount each cycle, chasing the current tape
     * length. The Servo travels 360 degrees in 72 seconds 50 degrees per second
     * or 1 degree per minute The angular range is about 270 degrees as servo
     * inpute goes from 0 to 1 which translates into about 0.0037 servo units
     * per period. This is ath maximum velocity of 50 degrees per second, way
     * too fast. Propose slowing it down by a factor of 4, so the incremental
     * adjustment is about .0009. A reasonable error value is 1.5 degree say
     * about 0.005 PID is possible, if the setpoint is not dynamic, but
     * unnecessary, I think
     *
     * @param serVel
     * @param goalVal
